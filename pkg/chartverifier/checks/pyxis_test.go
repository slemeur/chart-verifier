...
func Test_checkImageInRegistry(t *testing.T) {
	...
	PassTestCases := []testCase{
		{description: "Test nginx registry and version", repository: "nginx", registry: "registry.hub.docker.com", version: "latest", message: ""},
		{description: "Test nginx rhel6.7 and version", repository: "rhel6.7", registry: "registry.access.redhat.com", version: "6.7", message: ""},
		{description: "Test rhel8/nginx-116 respository", repository: "rhel8/nginx-116", registry: "registry.access.redhat.com", version: "1-75", message: ""},
		{description: "Test turbonomic/zookeeper respository and version", repository: "turbonomic/zookeeper", registry: "registry.connect.redhat.com", version: "8.1.2", message: ""},
	}

	for _, tc := range PassTestCases {
		t.Run(tc.description, func(t *testing.T) {
			found, err := checkImageInRegistry(tc.repository, tc.version, tc.registry)
			require.NoError(t, err)
