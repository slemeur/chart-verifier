...
func (e Engine) Render(chrt *chart.Chart, values chartutil.Values) (map[string]string, error) {
	fmt.Print("Process all templates \n")
	tmap := allTemplates(chrt, values)
	fmt.Print("Render the chart \n")
...
func (e Engine) initFunMap(t *template.Template, referenceTpls map[string]renderable) {
	...
	funcMap["include"] = func(name string, data interface{}) (string, error) {
		fmt.Printf("  include function : %s  \n",name)
		var buf strings.Builder
		if strings.HasPrefix(path.Base(name), "_") {
			fmt.Printf("    include function ignore : %s \n",name)
		...
		fmt.Printf("    Execute template %s \n",name)
		err := t.ExecuteTemplate(&buf, name, data)
		includedNames[name]--
		if err != nil {
			fmt.Printf("    Failed execute template : %v\n",err)
		} else {
			fmt.Printf("    Passed execute template : %s ::: \n", buf.String())
		}
		return buf.String(), nil
	}
	...
	funcMap["tpl"] = func(tpl string, vals chartutil.Values) (string, error) {
		...
		fmt.Printf("    Render with references : \n %v",templates)
	...
	funcMap["required"] = func(warn string, val interface{}) (interface{}, error) {
		fmt.Printf("  required function : %s \n",warn)
		if val == nil {
			if e.LintMode {
...
func (e Engine) renderWithReferences(tpls, referenceTpls map[string]renderable) (rendered map[string]string, err error) {
	...
	for _, filename := range keys {
		...
		if err := t.ExecuteTemplate(&buf, filename, vals); err != nil {
			fmt.Printf(" - Execute template error : %s : %v \n", filename,err)
...
func recAllTpls(c *chart.Chart, templates map[string]renderable, vals chartutil.Values) {
	...
	if c.IsRoot() {
		next["Values"] = vals
		fmt.Printf("recAllTps c is Root\n")
	} else if vs, err := vals.Table("Values." + c.Name()); err == nil {
		fmt.Printf("recAllTps values are Tabled\n")
		next["Values"] = vs
	}

	for _, child := range c.Dependencies() {
		fmt.Printf("recAllTps child : %v", child)
		recAllTpls(child, templates, next)
	}

	newParentID := c.ChartFullPath()
	for _, t := range c.Templates {
		fmt.Printf("recAllTps template 1 : %s\n", t.Name)
		if !isTemplateValid(c, t.Name) {
			continue
		}
		fmt.Printf("recAllTps template 2 : %s\n", t.Name)
